...
from Components import model_constants as mc
...
def make_trg_mask(trg):
    ...
    trg_pad_mask = (trg != mc.TRG_PAD_IDX).unsqueeze(1).unsqueeze(2)
...
def main():
    ...
    enc = Encoder(mc.INPUT_DIM, mc.HID_DIM, mc.ENC_LAYERS, mc.ENC_HEADS,
                  mc.ENC_PF_DIM, mc.ENC_DROPOUT, env.DEVICE)

    dec = Decoder(mc.OUTPUT_DIM, mc.HID_DIM, mc.DEC_LAYERS, mc.DEC_HEADS,
                  mc.DEC_PF_DIM, mc.DEC_DROPOUT, env.DEVICE)

    # SRC_PAD_IDX = Input.vocab.stoi[Input.pad_token]
    # TRG_PAD_IDX = Output.vocab.stoi[Output.pad_token]

    model = Seq2Seq(enc, dec, mc.SRC_PAD_IDX, mc.TRG_PAD_IDX, env.DEVICE).to(env.DEVICE)
    model.apply(initialize_weights)

    optimizer = torch.optim.Adam(model.parameters(), lr=mc.LEARNING_RATE)

    criterion = maskNLLLoss

    best_valid_loss = float('inf')

    for epoch in range(mc.N_EPOCHS):
        ...
        for i in range(train_df.shape[0]):
            try:
                ex = data.Example.fromlist([train_df.question[i], train_df.solution[i]], mc.fields)
        ...
        for i in range(val_df.shape[0]):
            try:
                ex = data.Example.fromlist([val_df.question[i], val_df.solution[i]], mc.fields)
                val_example.append(ex)
            except:
                pass

        train_data = data.Dataset(train_example, mc.fields)
        valid_data = data.Dataset(val_example, mc.fields)

        train_iterator, valid_iterator = BucketIterator.splits((train_data, valid_data), batch_size=mc.BATCH_SIZE,
                                                               sort_key=lambda x: len(x.Input),
                                                               sort_within_batch=True, device=env.DEVICE)

        train_loss = train(model, train_iterator, optimizer, criterion, mc.CLIP)
